  ...
  return (
    <aside className="lg:sticky lg:top-0 lg:h-screen lg:w-[480px] lg:flex-shrink-0 flex flex-col justify-between px-6 py-12 lg:px-16 lg:py-24">
      <div>
        ...
        <p className="mt-3 text-base text-text-muted">{data.tagline}</p>
        ...
        >
          {["about", "experience", "projects"].map((s) => (
            <a
              key={s}
              href={`#${s}`}
              className="text-xs font-semibold uppercase tracking-widest text-text-muted hover:text-text-bright border border-accent-border rounded-full px-4 py-2 transition"
            >
              {s}
            </a>
          ))}
